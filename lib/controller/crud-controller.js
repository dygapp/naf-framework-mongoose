...
const { isArray, isObject, isString, trimData, isUndefined, isNumber } = require('naf-core').Util;
...
MapOptions = (ctx, opts) => {
  ...
  if (!isUndefined(params.skip) && !isNumber(params.skip)) params.skip = Number(params.skip);
  if (!isUndefined(params.limit) && !isNumber(params.limit)) params.limit = Number(params.limit);
      ...
      if (!cls.prototype[key]) {
        ...
        cls.prototype[key] = async function() {
          ...
          if (_options && _options.count) {
            const funcName = isString(_options.count) ? _options.count : 'count';
            const func = provider[funcName];
            if (!is.asyncFunction(func)) {
